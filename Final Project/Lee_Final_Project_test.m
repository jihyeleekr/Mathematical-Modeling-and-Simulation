...
M = zeros(N,1);
quarantine_flag = zeros(size(I));
% Math to find base chance for agent in square shared by infected agent to become infected
r0 = 2.5; % amount of agents a single infected agent will infect
...
for j = 1:ns % loop over steps
    ...
    for k = 1:size(cells_with_multiple_agents,1) % for every cell with adequate number of agents
        ...
        if covid_present % if COVID is present in this cell
            for l = 1:size(agents_in_this_cell) % find the agents without COVID, and with a probability of the base COVID probability times the adjusted odds ratio 
                if ~I(agents_in_this_cell(l))>=1 && rand<p_base % of the specific type of mask they were using
    ...
    for k = 1:size(I) % check every agent
        if I(k) > 6 % if agent has been infected for 6 days
            % Move agent to quarantine (remove from plotting)
            M(k) = 0;
            % Reset agent's infection duration
            I(k) = 0;
            % Set agent's quarantine flag to indicate they are in quarantine
            quarantine_flag(k) = 1;
        elseif quarantine_flag(k) == 1 % if agent is in quarantine
            % Check if quarantine period is over (you need to define quarantine_over)
            if I(k) > quarantine_over
                % Agent is out of quarantine, reappear on plotting
                M(k) = 1;
                % Reset quarantine flag
                quarantine_flag(k) = 0;
            else
                % Increment quarantine duration
                I(k) = I(k) + 1;
            end
        elseif I(k) >= 1 % if agent is infected
    ...
    X = X(ind,:);D = D(ind); I = I(ind); % "On average, symptoms showed up in the newly infected person about 5.6 days"
    
    
    current_size = size(X,1) - nnz(I >= max_infection);
    
    if rand<new % With certain probability,
        X = [X;ax(1)+(ax(2)-ax(1))*rand, ax(3)+(ax(4)-ax(3))*rand];D = [D;rand*2*pi];I = [I;1]; %  add an agent that has COVID
    end
    ...
    % Plot immune agents
    ind = M==1;
    plot(X(ind,1),X(ind,2),'.','markersize',12,'color',[0.3 0.9 0.3], 'DisplayName', sprintf('Immune (%d)',nnz(ind)))
