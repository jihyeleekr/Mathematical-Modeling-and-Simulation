...
%   gosper_gun.txt, gosper_gun_reflected.txt, dart_rotated_multi.txt
%   to run. It shows two glider guns firing at triangle shaped darts 
%
...
filename{1} = 'gosper_gun.txt';
pos{1} = [5,5]; % position of object loaded from file (top left  corner)
filename{2} = 'dart_rotated_multi.txt';
pos{2} = [120,90]; % position of object loaded from file (bottom middle)
filename{3} = 'gosper_gun_reflected.txt';
pos{3} = [245,5]; % position of object loaded from file (top right corner)
...
for j = 0:3000 % time loop
    ...
    title(sprintf('Game of life in space war after %d steps',j))
