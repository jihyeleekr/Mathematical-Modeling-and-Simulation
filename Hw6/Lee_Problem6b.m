...
%            http://www.math.temple.edu/~seibold/ō

% Parameters
n = [250,100]; % number of cells per dimension
filename{1} = 'shuttle_left.txt';
pos{1} = [5,55]; % position of object loaded from file 
filename{2} = 'black_hole.txt';
pos{2} = [50,70]; % position of object loaded from file 
filename{3} = 'black_hole.txt';
pos{3} = [100, 30];
filename{4} = 'shuttle_right.txt';
pos{4} = [10, 25];
...
for j = 0:3000 % time loop
    ...
    title(sprintf('Game of life in space after %d steps',j))
