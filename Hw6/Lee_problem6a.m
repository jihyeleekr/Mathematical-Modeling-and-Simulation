...
filename{1} = 'eater_fishhook.txt';
pos{1} = [100,69];
filename{2} = 'mediumfish.txt';
pos{2} = [5,70];
filename{3} = 'mediumfish.txt';
pos{3} = [215,80];
filename{4} = 'mediumfish.txt'; % position of sinking boat (left bottom)
pos{4} = [40,70];
...
for j = 0:3000 % time loop
    ...
    title(sprintf('Game of life under water after %d steps',j))
